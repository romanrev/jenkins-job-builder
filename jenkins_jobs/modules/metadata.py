...
"""
The Metadata plugin module enables to add metadata to the projects
...
def base_metadata(parser, xml_parent, data, do_default, mtype):
    ...
    exposed = data.get('exposeToEnv', False)
    if exposed:
        exposed_to_env.text = 'true'
    else:
        exposed_to_env.text = 'false'
    return pdef
...
def string_metadata(parser, xml_parent, data):
    ...
    :arg bool exposeToEnv: exposure of the metadata  to environment (optional)
...
def date_metadata(parser, xml_parent, data):
    ...
    :arg str name: the name of the metadata
    :arg str time: time value of the metadata
